...
- 2026-06-25 已刷入并验证 `0.1.71` 配置页和 BYOA 配对体验修正：
  - 配置页将“保存”和“重启”合并为 `保存并重启`，危险操作折叠，串口输出
    默认折叠，降低普通用户配置复杂度。2026-07-01 起折叠区命名为
    `重置配置`，并按上方接管方式显示 `重置微信登录` 或 `重置智能体配对`；
    点击后会清对应凭证、保存当前接管方式并重启。
  - 已保存 Wi-Fi 密码不会被读回明文；配置页显示“已保存，留空不修改”，
    SSID 未变且密码为空时只发送 `agent_mode`，不会覆盖 NVS 里的密码。
  - 新增串口命令 `WEC:CLEAR_AGENT`，配置页 `重置智能体配对` 会清除 BYOA
    凭据，便于重新显示配对码。
